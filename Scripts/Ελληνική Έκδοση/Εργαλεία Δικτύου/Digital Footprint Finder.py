...
Εργαλείο OSINT για έρευνες βασισμένες σε δημόσιες πηγές, για νόμιμη και εξουσιοδοτημένη χρήση.

Στόχοι σχεδίασης
----------------
- Ενιαίο αρχείο Python με αυτοσυντηρούμενες προαιρετικές δυνατότητες εμπλουτισμού.
- Εύκολη λειτουργία: εισάγετε έναν στόχο και δημιουργείται αυτόματα υπόθεση και αναφορά.
- Ροή εργασίας οργανωμένη ανά υπόθεση με βάση SQLite.
- Γράφος οντοτήτων και σχέσεων αντί για αποσυνδεδεμένα αποτελέσματα αναζήτησης.
- Καταγραφή αποδεικτικών στοιχείων με SHA-256 και χρονικές σημάνσεις UTC.
- Εγγενείς έρευνες δημόσιων πηγών για domains, URLs, usernames, emails, IPs, ονόματα, τηλέφωνα και τοπικά αρχεία.
- Δημόσια APIs χωρίς κλειδί από προεπιλογή, συν προαιρετικά API credentials που μπορεί να ορίσει ο χρήστης.
- Ενσωματωμένη ανίχνευση ψευδώς θετικών προφίλ, παθητική ανακάλυψη hosts, περιορισμένο crawling, ανακάλυψη JS endpoints και συσχέτιση.
- Βαθμός εμπιστοσύνης μεταξύ ανεξάρτητων πηγών, αντιφάσεις, χρονογραμμές, διαδρομές σχέσεων, ομάδες συσχέτισης και έλεγχοι ακεραιότητας αποδεικτικών στοιχείων.
- Εξαγωγές Markdown, JSON, CSV, GraphML, STIX 2.1, ακατέργαστων αποδεικτικών, ZIP πακέτου και αυτοτελούς HTML.
- Πλήρης υποστήριξη χώρου εργασίας Termux και φακέλων υποθέσεων στις Λήψεις.
- Λειτουργεί σε Termux, Linux, macOS και Windows με Python 3.10+.

Σημαντικό
---------
Το πρόγραμμα προορίζεται για νόμιμο OSINT, εκπαίδευση κυβερνοασφάλειας,
δημοσιογραφία, due diligence και έρευνες σε πληροφορίες στις οποίες έχετε
εξουσιοδοτημένη πρόσβαση. Δεν παρακάμπτει authentication, δεν αποκτά πρόσβαση
σε ιδιωτικούς λογαριασμούς, δεν ανακτά κλεμμένα credentials και δεν παρακάμπτει
μηχανισμούς ελέγχου πρόσβασης.

Ορισμένες δημόσιες υπηρεσίες μπορεί να εφαρμόζουν rate limits ή να αλλάζουν συμπεριφορά.
Τα αποτελέσματα πρέπει να θεωρούνται ερευνητικές ενδείξεις και να επαληθεύονται ανεξάρτητα.
"""
...
# ---------------------------------------------------------------------------
# Διατηρείτε τα URLs παρόχων και τα πρότυπα ιστοτόπων συγκεντρωμένα ώστε οι αλλαγές upstream να διορθώνονται εύκολα.
# Το Digital Footprint Finder υλοποιεί εγγενώς τις ερευνητικές του δυνατότητες. ΔΕΝ πρέπει να κάνει clone,
# εγκατάσταση, import ή εκτέλεση τρίτων OSINT projects. Παρόμοιες χρήσιμες τεχνικές
# υλοποιούνται ανεξάρτητα παρακάτω ώστε ένα αρχείο να παραμένει ολόκληρη η μηχανή έρευνας.
# Η υποχρεωτική λειτουργικότητα πρέπει να συνεχίζει να λειτουργεί με την Python standard library.
# Τα μικρά προαιρετικά Python packages εγκαθίστανται στο ~/Digital Footprint Finder/dependencies και ποτέ globally.
# Κάθε δημόσια πηγή πρέπει να αποτυγχάνει ομαλά: ένα μη διαθέσιμο API δεν πρέπει να διακόπτει μια υπόθεση.
# Ποτέ μη θεωρείτε ένα αποτέλεσμα αναζήτησης/API/profile ως επιβεβαιωμένη ταυτότητα χωρίς ανεξάρτητη διασταύρωση.
# Το confidence αφορά την αξιοπιστία μιας παρατήρησης και όχι δήλωση ότι δύο εγγραφές αφορούν το ίδιο άτομο.
# Το crawling πρέπει να είναι ευγενικό: μόνο δημόσιες σελίδες, σεβασμός στο robots.txt, όρια σελίδων και χωρίς auth/session.
# Μην προσθέτετε λειτουργίες credentials, breach dumps, εξαγωγής μυστικών, authentication bypass ή exploits.
# Διατηρείτε σταθερή τη δομή αποθήκευσης: Downloads/Digital Footprint Finder/<Όνομα Υπόθεσης>/files.
...
# Τα API keys είναι προαιρετικά. Το εργαλείο παραμένει πλήρως λειτουργικό με δημόσιες πηγές χωρίς κλειδί.
# Τα κλειδιά δεν γράφονται ποτέ σε αναφορές/evidence και αποκρύπτονται στο UI.
API_PROVIDERS = {
    "github": {
        "label": "GitHub",
        "env": ("DIGITAL_FOOTPRINT_FINDER_GITHUB_TOKEN", "GITHUB_TOKEN"),
        "purpose": "Υψηλότερα όρια REST API για εμπλουτισμό δημόσιων δεδομένων GitHub",
    },
    "virustotal": {
        "label": "VirusTotal",
        "env": ("DIGITAL_FOOTPRINT_FINDER_VIRUSTOTAL_API_KEY", "VIRUSTOTAL_API_KEY"),
        "purpose": "Δημόσιο reputation/context για domains, IPs, URLs και SHA-256 hashes",
    },
    "urlscan": {
        "label": "urlscan.io",
        "env": ("DIGITAL_FOOTPRINT_FINDER_URLSCAN_API_KEY", "URLSCAN_API_KEY"),
        "purpose": "Αναζήτηση υπάρχοντος δημόσιου ιστορικού website scans· δεν υποβάλλει νέα scans",
    },
    "shodan": {
        "label": "Shodan",
        "env": ("DIGITAL_FOOTPRINT_FINDER_SHODAN_API_KEY", "SHODAN_API_KEY"),
        "purpose": "Παθητικές δημόσιες παρατηρήσεις host/service για δημόσιες IP διευθύνσεις",
    },
    "openalex": {
        "label": "OpenAlex",
        "env": ("DIGITAL_FOOTPRINT_FINDER_OPENALEX_API_KEY", "OPENALEX_API_KEY"),
        "purpose": "Προαιρετική authenticated πρόσβαση σε ακαδημαϊκό γράφο",
    },
}
...
KEYLESS_PUBLIC_API_CATALOG = [
    {"name": "Google Public DNS", "purpose": "Εφεδρικό DNS-over-HTTPS και επίλυση DNS records"},
    {"name": "RDAP.org", "purpose": "Δημόσια δεδομένα καταχώρισης domain και IP"},
    {"name": "crt.sh", "purpose": "Παρατηρήσεις hostnames από Certificate Transparency"},
    {"name": "Internet Archive CDX", "purpose": "Ιστορικά δημόσια URLs και hostnames"},
    {"name": "GitHub REST API", "purpose": "Δημόσια profiles, repositories και υποψήφια ονόματα χαμηλής εμπιστοσύνης"},
    {"name": "GitLab API", "purpose": "Υποψήφια δημόσια profiles"},
    {"name": "ipwho.is", "purpose": "Κατά προσέγγιση δημόσιο network/location context για IP"},
    {"name": "RIPEstat", "purpose": "Context για ASN, prefixes, reverse DNS και routing"},
    {"name": "HackerTarget", "purpose": "Παθητικές παρατηρήσεις host και reverse-IP εντός των ορίων του παρόχου"},
    {"name": "AlienVault OTX", "purpose": "Παθητικές DNS παρατηρήσεις"},
    {"name": "Common Crawl", "purpose": "Ιστορικές παρατηρήσεις από δημόσια web indexes"},
    {"name": "Wikidata", "purpose": "Δημόσια υποψήφια πρόσωπα και οργανισμοί"},
    {"name": "Crossref", "purpose": "Δημόσιο ακαδημαϊκό context δημοσιεύσεων/συγγραφέων"},
    {"name": "ROR", "purpose": "Δημόσιο identity context ερευνητικών οργανισμών"},
    {"name": "OpenAlex", "purpose": "Δημόσιο ακαδημαϊκό context συγγραφέων/ιδρυμάτων"},
    {"name": "Stack Exchange", "purpose": "Δημόσια community profiles χαμηλής εμπιστοσύνης"},
]
...
# Προαιρετικά enrichers σε καθαρή Python. Αν η εγκατάσταση αποτύχει, η αντίστοιχη λειτουργία υποβαθμίζεται ομαλά.
BOOTSTRAP_PACKAGES = {
    "bs4": "beautifulsoup4",
    "dns": "dnspython",
...
def ensure_workspace() -> None:
    """Δημιουργεί τον μόνιμο χώρο εργασίας του Digital Footprint Finder."""
...
def load_settings() -> dict[str, Any]:
    """Φορτώνει μόνιμες ρυθμίσεις και συγχωνεύει μελλοντικές προεπιλογές χωρίς να χαλά παλιά αρχεία."""
    ensure_workspace()
    data: dict[str, Any] = {}
...
def load_api_keys() -> dict[str, str]:
    """Φορτώνει προαιρετικά κλειδιά παρόχων. Άγνωστα πεδία αγνοούνται και τα secrets δεν εξάγονται ποτέ."""
    ensure_workspace()
...
def api_provider_status() -> dict[str, dict[str, Any]]:
    ...
    for name, meta in API_PROVIDERS.items():
        ...
        out[name] = {
            ...
            "source": "μεταβλητή περιβάλλοντος" if env_source else ("ασφαλές τοπικό αρχείο" if saved.get(name) else "μη ρυθμισμένο"),
...
def api_status_report() -> dict[str, Any]:
    return {
        ...
        "privacy": "Οι τιμές των API credentials δεν εμφανίζονται ποτέ και δεν εξάγονται στις αναφορές υποθέσεων.",
    }
...
def redact_url_secrets(url: str) -> str:
    """Αφαιρεί συνηθισμένες παραμέτρους credentials από URLs πριν εμφανιστούν ή αποθηκευτούν."""
    try:
...
def api_keys_menu() -> None:
    while True:
        ...
        UI.title("Προαιρετικά APIs")
        names = list(API_PROVIDERS)
        for i, name in enumerate(names, 1):
            row = status[name]
            state = "ρυθμισμένο" if row["configured"] else "μη ρυθμισμένο"
            print(f"[{i}] {row['label']:<16} {state:<16} {row['purpose']}")
        print("[R] Αφαίρεση αποθηκευμένου API key")
        print("[0] Πίσω")
        raw = input("Επιλέξτε πάροχο: ").strip().lower()
        if raw in {"", "0"}:
            return
        if raw == "r":
            target = input("Όνομα παρόχου για αφαίρεση: ").strip().lower()
            if target not in API_PROVIDERS:
                UI.err("Άγνωστος πάροχος.")
                continue
            keys = load_api_keys(); keys.pop(target, None); save_api_keys(keys); UI.ok("Το αποθηκευμένο κλειδί αφαιρέθηκε.")
            continue
        try:
            provider = names[int(raw)-1]
        except (ValueError, IndexError):
            UI.err("Μη έγκυρη επιλογή.")
            continue
        value = getpass.getpass(f"{API_PROVIDERS[provider]['label']} API key/token (η εισαγωγή είναι κρυφή): ").strip()
        if not value:
            UI.warn("Δεν έγινε καμία αλλαγή.")
            continue
        keys = load_api_keys(); keys[provider] = value; save_api_keys(keys); UI.ok("Το API credential αποθηκεύτηκε τοπικά με περιορισμένα δικαιώματα πρόσβασης.")
...
def ensure_termux_storage() -> Optional[Path]:
    """Επιστρέφει τον φάκελο Downloads του Termux και ζητά άδεια αποθήκευσης Android όταν απαιτείται."""
...
def bootstrap_dependencies(verbose: bool = True) -> dict[str, str]:
    """
    Εγκαθιστά τοπικά μικρά προαιρετικά enrichers όταν λείπουν.

    Δεν εγκαθίσταται τίποτα στο global περιβάλλον Python/Termux. Το script συνεχίζει
    να λειτουργεί ακόμη κι αν η εγκατάσταση δεν είναι διαθέσιμη· η διαδικασία είναι σκόπιμα best-effort.
    """
    ensure_workspace()
    status: dict[str, str] = {}
    ...
    for module, package in BOOTSTRAP_PACKAGES.items():
        if _module_available(module):
            ...
            continue
        if verbose:
            print(f"[ρύθμιση] Προσθήκη προαιρετικής δυνατότητας: {package}")
...
def case_files_dir(case_name: str) -> Path:
    """Downloads/Digital Footprint Finder/<Όνομα Υπόθεσης>/files, με fallback στο home αν δεν υπάρχει άδεια."""
...
def normalize_url(value: str) -> str:
    ...
    if parsed.scheme not in {"http", "https"}:
        raise ValueError("Υποστηρίζονται μόνο URLs http:// και https://.")
...
def extract_document_metadata_bytes(body: bytes, suffix: str) -> dict[str, Any]:
    """Εξάγει metadata από περιορισμένο δημόσιο document body χωρίς εγγραφή στον δίσκο."""
    suffix = suffix.lower()
...
def extract_page_intelligence(base_url: str, body: bytes) -> dict[str, Any]:
    """Εξάγει συντηρητικά, ρητά δημόσια metadata σελίδας και contact leads."""
...
def detect_target_type(target: str) -> str:
    """Best-effort ταξινομητής στόχου για την Εύκολη Λειτουργία."""
    target = target.strip()
    if not target:
        return "unknown"
...
def strip_explicit_target_prefix(target: str) -> str:
    """Αφαιρεί φιλικά προς τον χρήστη prefixes όπως org:, doi:, asn:, prefix:, hash:."""
    value = target.strip()
    lowered = value.lower()
...
def auto_case_name(target: str) -> str:
    """Χρησιμοποιεί σταθερό, αναγνώσιμο όνομα υπόθεσης ώστε επαναλαμβανόμενες εκτελέσεις να συνεχίζουν την ίδια υπόθεση."""
...
def default_report_dir(case_name: str = "General") -> Path:
    """Βοηθητική συνάρτηση συμβατότητας· νέος κώδικας πρέπει να χρησιμοποιεί case_files_dir(case_name)."""
    return case_files_dir(case_name)


def open_local_file(path: Path) -> bool:
    """Best-effort άνοιγμα με την προεπιλεγμένη εφαρμογή της πλατφόρμας."""
    try:
...
def http_get(
    ...
    """Περιορισμένο GET με μικρά retries για προσωρινές αποτυχίες δημόσιων παρόχων."""
...
def http_json_lines(url: str, timeout: int = DEFAULT_TIMEOUT, max_bytes: int = 4_000_000) -> list[dict[str, Any]]:
    """Διαβάζει newline-delimited JSON που χρησιμοποιείται από δημόσια indexes όπως το Common Crawl."""
...
def run_dig(name: str, record_type: str) -> list[str]:
    """Επιλύει DNS με τοπικό dig όταν υπάρχει και διαφορετικά χρησιμοποιεί Google Public DNS DoH JSON."""
...
def _normalized_page_sample(body: bytes, limit: int = 80_000) -> str:
    """Κανονικοποιεί αρκετό HTML ώστε να συγκρίνει πραγματική απόκριση profile με fake-profile baseline."""
...
def _extract_js_endpoints(js_url: str, body: bytes, limit: int = 120) -> list[str]:
    """Εξάγει δημόσια URL/path strings από JavaScript· σκόπιμα δεν αναζητά secrets."""
...
def _parse_sitemap_urls(xml_text: str, limit: int = 500) -> list[str]:
    """Αναλύει sitemap indexes και URL sets χωρίς απαίτηση πρόσθετου XML dependency."""
    out: list[str] = []
    try:
...
def detect_technology_hints(headers: dict[str, str], body: bytes, page_intelligence: Optional[dict[str, Any]] = None) -> dict[str, Any]:
    """Συντηρητικό fingerprinting από δημόσια headers/HTML· τα ευρήματα είναι ενδείξεις, όχι εγγυήσεις."""
...
def tls_certificate_info(domain: str, timeout: int = 8) -> dict[str, Any]:
    """Διαβάζει το πιστοποιητικό δημόσιας HTTPS υπηρεσίας μέσω κανονικού TLS handshake."""
...
def conservative_confidence(old: float, incoming: float, old_sources: str, new_source: str) -> float:
    """Αυξάνει την εμπιστοσύνη μόνο όταν υπάρχει διασταύρωση από πραγματικά διαφορετική οικογένεια πηγών."""
...
class CaseDB:
    ...
    def add_entity(
        ...
        *,
        ...
        """Προσθέτει/συγχωνεύει οντότητα και ενισχύει συντηρητικά την ανεξάρτητη διασταύρωση."""
        typ = typ.lower().strip()
    ...
    def add_assessment(self, case_id: int, entity_id: int, status: str, note: str = "") -> int:
        ...
        if status not in {"confirmed", "rejected", "uncertain"}:
            raise ValueError("Η αξιολόγηση πρέπει να είναι confirmed, rejected ή uncertain.")
        cur = self.conn.execute(
    ...
    def search(self, query: str, *, case_id: Optional[int] = None, limit: int = 25) -> list[dict[str, Any]]:
        """Έξυπνη τοπική αναζήτηση με προαιρετικά φίλτρα: kind:, type:, source:, confidence:, id:, case:."""
        raw=query.strip()
        if not raw: return []
...
class DigitalFootprintFinder:
    ...
    def _stackexchange_candidates(self, value: str, root: int, limit: int = 6) -> list[dict[str, Any]]:
        """Υποψήφια δημόσια profiles Stack Exchange χωρίς κλειδί. Αντιστοιχίσεις ονόματος/alias παραμένουν ενδείξεις χαμηλής εμπιστοσύνης."""
        out: list[dict[str, Any]] = []
        try:
            ...
            for row in ((data or {}).get("items") or [])[:limit]:
                ...
                if profile:
                    ...
                    self.db.add_relation(self.case_id, root, uid, "possible_public_candidate", confidence=0.34, source="αναζήτηση Stack Exchange μόνο βάσει ονόματος/alias· απαιτείται επαλήθευση ταυτότητας")
                if row.get("website_url"):
                    with contextlib.suppress(Exception):
                        ...
                        self.db.add_relation(self.case_id, root, wid, "candidate_public_website", confidence=0.30, source="υποψήφιο Stack Exchange· απαιτείται επαλήθευση ταυτότητας")
    ...
    def optional_api_enrichment(self, typ: str, value: str, root: int) -> dict[str, Any]:
        """Εκτελεί μόνο ρυθμισμένους προαιρετικούς παρόχους και επιστρέφει δημόσιο enrichment χωρίς έκθεση credentials."""
        out: dict[str, Any] = {}
    ...
    def _attach_search_leads(self, root_id: int, links: list[dict[str, str]]) -> None:
        for row in links:
            ...
            uid = self._entity(
                ...
                confidence=0.25,
                source="παραγόμενο ερώτημα δημόσιας αναζήτησης",
            )
            self.db.add_relation(
                self.case_id, root_id, uid, "manual_search_lead",
                confidence=0.25, source="παραγόμενο ερώτημα· δεν αποτελεί αντιστοίχιση ταυτότητας",
            )
    ...
    def _enrich_public_profile(self, root: int, site: str, url: str, confidence: float) -> dict[str, Any]:
        """Εξάγει περιορισμένα δημόσια metadata/links από σελίδα profile· χωρίς login ή cookies."""
        try:
    ...
    def _probe_username_site(self, item: tuple[str, str], username: str) -> dict[str, Any]:
        ...
        if baseline is None:
            ...
            return {"site": site_name, "url": url, "http_status": target.status,
                    ...
                    "method": "επιτυχία υποψηφίου· baseline μη διαθέσιμο"}
        ...
        if baseline.status in {404, 410} or baseline_negative:
            return {"site": site_name, "url": url, "http_status": target.status,
                    ...
                    "method": "επιτυχία υποψηφίου + αρνητικό fake-user baseline"}
        ...
        if same_destination and similarity > 0.92:
            result, conf, method = "unknown", 0.15, "ο υποψήφιος και ο fake user καταλήγουν στην ίδια σελίδα"
        elif mentioned and similarity < 0.93:
            result, conf, method = "probable", 0.68, "το username εμφανίζεται και διαφέρει από τη σελίδα fake-user"
        elif similarity < 0.72:
            result, conf, method = "possible", 0.52, "ο υποψήφιος διαφέρει ουσιαστικά από τη σελίδα fake-user"
        else:
            result, conf, method = "unknown", 0.18, "ο ιστότοπος επιστρέφει αμφίσημες επιτυχημένες σελίδες"
    ...
    def _github_repositories(self, username: str, root: int) -> list[dict[str, Any]]:
        """Συλλέγει περιορισμένο σύνολο δημόσιων GitHub repositories ως διασταυρωτικό δημόσιο context."""
    ...
    def _hackertarget_hosts(self, domain: str, root: int) -> list[dict[str, str]]:
        """Χρησιμοποιεί το δωρεάν passive host-search endpoint του HackerTarget· δεν πραγματοποιείται scanning."""
    ...
    def _otx_passive_dns(self, domain: str, root: int) -> list[dict[str, Any]]:
        """Διαβάζει δημόσιες passive-DNS παρατηρήσεις από OTX όταν το endpoint επιτρέπει ανώνυμη πρόσβαση."""
    ...
    def _wayback_hosts(self, domain: str, root: int) -> list[str]:
        """Παράγει ιστορικά hostnames από αρχειοθετημένα URLs ως παθητική πηγή subdomains."""
    ...
    def passive_domain_intelligence(self, domain: str, root: int) -> dict[str, Any]:
        """Εγγενής παθητική ανακάλυψη hosts από ανεξάρτητες δημόσιες πηγές."""
        return {
    ...
    def domain_mail_posture(self, domain: str, root: int) -> dict[str, Any]:
        """Ελέγχει δημόσια email-delivery/security records και MTA-STS policy."""
    ...
    def _common_crawl_domain(self, domain: str, root: int, limit: int = 120) -> dict[str, Any]:
        """Ερωτά το πιο πρόσφατο Common Crawl URL index χωρίς λήψη crawl bodies."""
    ...
    def _wikidata_candidates(self, query: str, root: int, entity_kind: str = "person") -> list[dict[str, Any]]:
        """Αναζήτηση υποψηφίων χαμηλής εμπιστοσύνης. Οι υποψήφιες εγγραφές παραμένουν ξεχωριστές από τον στόχο του χρήστη."""
        params = urllib.parse.urlencode({
        ...
        try:
            ...
            for row in (data.get("search", []) if isinstance(data, dict) else [])[:5]:
                ...
                self.db.add_relation(self.case_id, root, cid, f"possible_{entity_kind}_record",
                                     confidence=0.28, source="αναζήτηση Wikidata· επαληθεύστε τον υποψήφιο")
    ...
    def _crossref_author_candidates(self, name: str, root: int, rows: int = 6) -> list[dict[str, Any]]:
        """Αναζητά δημόσια ακαδημαϊκά metadata και διατηρεί τις αντιστοιχίσεις ρητά σε επίπεδο υποψηφίου."""
        ...
        try:
            ...
            for item in items[:rows]:
                ...
                self.db.add_relation(self.case_id, root, pid, "possible_author_of",
                                     ...
                                     source="αντιστοίχιση ονόματος Crossref· επαληθεύστε τη συγγραφική ταυτότητα")
                if item.get("URL"):
    ...
    def doi(self, value: str) -> dict[str, Any]:
        ...
        if not re.fullmatch(r"(?i)10\.\d{4,9}/[-._;()/:A-Z0-9]+", doi):
            raise ValueError("Εισαγάγετε έγκυρο DOI, π.χ. 10.1000/example")
    ...
    def prefix(self, value: str) -> dict[str, Any]:
        ...
        try:
            ...
            raise ValueError("Εισαγάγετε CIDR prefix όπως 1.1.1.0/24") from exc
        prefix = str(network)
    ...
    def hash_value(self, value: str) -> dict[str, Any]:
        ...
        if not re.fullmatch(r"[a-f0-9]{64}", raw):
            raise ValueError("Στην Εύκολη Λειτουργία αναγνωρίζονται μόνο SHA-256 hashes.")
        ...
        return {"algorithm": "SHA-256", "hash": raw, "dorks": dorks, "optional_apis": optional_apis,
                "note": "Το Digital Footprint Finder δεν πραγματοποιεί αναζητήσεις σε credential dumps ή ιδιωτικές βάσεις δεδομένων παραβιάσεων."}
    ...
    def organization(self, value: str) -> dict[str, Any]:
        ...
        return {"organization": name, "wikidata_candidates": wikidata, "ror_candidates": ror,
                ...
                "note": "Οι υποψήφιες εγγραφές παραμένουν ξεχωριστές μέχρι να επαληθευτούν ανεξάρτητα."}

    def _ror_candidates(self, name: str, root: int, limit: int = 8) -> list[dict[str, Any]]:
        """Επιλύει υποψήφια ονόματα οργανισμών μέσω του ανοικτού registry ROR v2."""
        out: list[dict[str, Any]] = []
        try:
            ...
            for row in items[:limit]:
                ...
                self.db.add_relation(self.case_id, root, oid, "possible_public_candidate", confidence=conf,
                                     source="αντιστοίχιση affiliation ROR· επαληθεύστε την ταυτότητα οργανισμού")
                if ror_id:
        ...
        try:
            ...
            for row in ((data or {}).get("results") or [])[:limit]:
                ...
                self.db.add_relation(self.case_id, root, aid, "possible_public_candidate", confidence=conf,
                                     source="αναζήτηση ονόματος OpenAlex· επαληθεύστε την ταυτότητα")
                if row.get("id"):
                ...
                for inst in institutions[:6]:
                    if inst.get("name"):
                        ...
                        self.db.add_relation(self.case_id, aid, inst_id, "known_institution_candidate", confidence=0.68,
                                             source="OpenAlex· επαληθεύστε την τρέχουσα affiliation")
    ...
    def _openalex_institution_candidates(self, name: str, root: int, limit: int = 6) -> list[dict[str, Any]]:
        ...
        try:
            ...
            for row in ((data or {}).get("results") or [])[:limit]:
                ...
                self.db.add_relation(self.case_id, root, oid, "possible_public_candidate", confidence=conf,
                                     source="αναζήτηση ιδρύματος OpenAlex· επαληθεύστε την ταυτότητα")
                if row.get("ror"):
    ...
    def fediverse(self, value: str) -> dict[str, Any]:
        """Επιλύει ρητό Fediverse handle μέσω WebFinger και του δημόσιου actor document."""
        raw = strip_explicit_target_prefix(value).strip()
        raw = re.sub(r"^acct:", "", raw, flags=re.I).lstrip("@")
        if not re.fullmatch(r"[A-Za-z0-9_.-]{1,64}@[A-Za-z0-9.-]+\.[A-Za-z]{2,}", raw):
            raise ValueError("Χρησιμοποιήστε ρητό Fediverse handle όπως @alice@example.social")
    ...
    def dorks(self, target: str, typ: Optional[str] = None, attach_to: Optional[int] = None) -> dict[str, Any]:
        """Δημιουργεί ασφαλή search dorks δημόσιων πληροφοριών και τα αποθηκεύει ως ενδείξεις υπόθεσης."""
        target = target.strip()
    ...
    def _github_name_candidates(self, name: str, root: int) -> list[dict[str, Any]]:
        """Δημόσιοι υποψήφιοι χαμηλής εμπιστοσύνης. Ποτέ δεν προάγονται αυτόματα σε επιβεβαιωμένη ταυτότητα."""
    ...
    def domain(self, domain: str, deep: bool = True) -> dict[str, Any]:
        ...
        if not domain or "." not in domain:
            raise ValueError("Εισαγάγετε έγκυρο domain, π.χ. example.com")
            ...
            try:
                ...
                for san in tls.get("subject_alt_names", [])[:300]:
                    ...
                    if san == domain or san.endswith("." + domain):
                        sid = self._entity("domain", san, confidence=0.76, source="τρέχον SAN πιστοποιητικού TLS")
    ...
    def ip(self, value: str) -> dict[str, Any]:
        ...
        if is_public_ip(ip):
                    ...
                    for line in _response_text(rr.body, 400_000).splitlines()[:200]:
                        ...
                        if host and "." in host and not host.lower().startswith(("error", "api count")):
                            ...
                            self.db.add_relation(self.case_id, eid, hid, "shared_ip_observation",
                                                 confidence=0.38, source="HackerTarget· πιθανό shared hosting")
            ...
            result["errors"].append("Ιδιωτική/δεσμευμένη διεύθυνση: παραλείφθηκε το εξωτερικό RDAP lookup.")
        return result
    ...
    def alias_variants(self, value: str, limit: int = 24) -> list[str]:
        """Δημιουργεί συντηρητικές παραλλαγές username/alias μόνο ως ενδείξεις· δεν τις ελέγχει αυτόματα."""
    ...
    def username(self, username: str, workers: int = 8) -> dict[str, Any]:
        ...
        if not re.fullmatch(r"[A-Za-z0-9_.-]{1,64}", username):
            raise ValueError("Το username περιέχει μη υποστηριζόμενους χαρακτήρες.")
    ...
    def email(self, value: str) -> dict[str, Any]:
        ...
        if not m:
            raise ValueError("Μη έγκυρη μορφή διεύθυνσης email.")
    ...
    def person(self, name: str) -> dict[str, Any]:
        ...
        return {
            ...
            "stackexchange_candidates": stackexchange_candidates,
            "note": "Οι αντιστοιχίσεις μόνο βάσει ονόματος είναι αμφίσημες. Οι υποψήφιες εγγραφές παραμένουν ξεχωριστές μέχρι να διασταυρωθούν από πολλαπλά ανεξάρτητα χαρακτηριστικά.",
        }
    ...
    def phone(self, value: str) -> dict[str, Any]:
        ...
        if not re.fullmatch(r"\+?\d{8,16}", compact):
            raise ValueError("Εισαγάγετε έναν εύλογο διεθνή ή εθνικό αριθμό τηλεφώνου.")
        ...
        return {
            ...
            "search_links": links,
            "dorks": dorks,
            "note": "Μόνο ενδείξεις από δημόσιες αναζητήσεις. Η αντιστοίχιση ενός αριθμού τηλεφώνου δεν αποδεικνύει ταυτότητα ή τρέχουσα κατοχή.",
        }
    ...
    def deep(self, target: str) -> dict[str, Any]:
        """Εύκολη Λειτουργία: ταξινομεί έναν στόχο και εκτελεί αυτόματα την ασφαλέστερη χρήσιμη έρευνα."""
        target = target.strip()
        if not target:
        ...
        if typ == "file":
            ...
            raise ValueError("Δεν ήταν δυνατό να αναγνωριστεί ο τύπος στόχου.")
    ...
    for eid, neighbors in adjacency.items():
        ...
        if len(neighbors) >= 2 and e["type"] in {"ip","domain","email","identifier","hash","asn","organization","technology"}:
            shared.append({"id":eid,"type":e["type"],"value":e["value"],"connections":len(neighbors),
                           "warning":"Κοινή υποδομή ή identifiers αποτελούν ενδείξεις συσχέτισης και όχι απόδειξη κοινής ιδιοκτησίας."})
        ...
        for rel, rows in groups.items():
            ...
            if len(vals)>1:
                out.append({"entity_id":sid,"entity":entities.get(sid,{}).get("value"),"relation":rel,
                            "values":vals,"severity":"review","note":"Πολλαπλές δημόσιες παρατηρήσεις διαφωνούν· μπορεί να πρόκειται για μετονομασία, παρωχημένα δεδομένα ή διαφορετική ταυτότητα."})
    return out
...
def investigation_quality(db: CaseDB, case_id: int) -> dict[str, Any]:
    ...
    return {"score":score,"label":label,"source_families":sorted(families),"corroborated_entities":corroborated,
            ...
            "note":"Αυτό μετρά την πληρότητα και την τεκμηρίωση της έρευνας, όχι υποψία, εγκληματικότητα ή κίνδυνο."}
...
def explain_entity(db: CaseDB, case_id: int, entity_id: int) -> dict[str, Any]:
    ...
    if not e: raise ValueError(f"Entity #{entity_id} δεν βρέθηκε σε αυτή την υπόθεση.")
    incoming=[]; outgoing=[]
    ...
    why=[]
    if len(families)>=3: why.append("Υποστηρίζεται από αρκετές ανεξάρτητες οικογένειες πηγών.")
    elif len(families)==1: why.append("Προς το παρόν εξαρτάται από μία μόνο οικογένεια πηγών· ανεξάρτητη διασταύρωση θα το ενίσχυε ουσιαστικά.")
    if len(incoming)+len(outgoing)>=5: why.append("Αυτή η οντότητα έχει πολλές συνδέσεις και μπορεί να αποτελεί σημαντική γέφυρα ή pivot στον γράφο.")
    if assessments: why.append(f"Τελευταία αξιολόγηση ερευνητή: {assessments[-1]['status']}.")
    next_steps=[]
    if len(families)<2: next_steps.append("Αναζητήστε διαφορετική οικογένεια πηγών πριν το θεωρήσετε ισχυρά διασταυρωμένο.")
    if not assessments: next_steps.append("Ελέγξτε χειροκίνητα αυτή την οντότητα και καταγράψτε επιβεβαιωμένο/απορριφθέν/αβέβαιο αν είναι σημαντική.")
...
def relationship_path(db: CaseDB, case_id: int, start_id: int, end_id: int) -> list[dict[str, Any]]:
    ...
    if start_id not in ents or end_id not in ents: raise ValueError("Και τα δύο IDs οντοτήτων πρέπει να υπάρχουν στην επιλεγμένη υπόθεση.")
    ...
    for e in d["entities"]:
        ...
        if len(fams)<2 and float(e["confidence"])>=0.55:
            ...
            out.append({"priority":priority,"entity_id":eid,
                        "question":f"Μπορεί το {e['type']} {e['value']} να διασταυρωθεί από διαφορετική οικογένεια πηγών;",
                        "why":f"Αυτή τη στιγμή διαθέτει {len(fams)} ανεξάρτητη/ες οικογένεια/ες πηγών και βαθμό συνδεσιμότητας {degree.get(eid,0)}."})
        if degree.get(eid,0)>=5 and eid not in assessments:
            out.append({"priority":70+min(15,degree[eid]),"entity_id":eid,
                        "question":f"Έχει ελεγχθεί χειροκίνητα η κεντρική οντότητα #{eid} ({e['value']});",
                        "why":"Οντότητες με πολλές συνδέσεις μπορούν να επηρεάσουν πολλά μεταγενέστερα συμπεράσματα."})
    for c in case_contradictions(db,case_id):
        out.append({"priority":100,"entity_id":c["entity_id"],"question":f"Ποια από τις αντικρουόμενες παρατηρήσεις {c['relation']} υποστηρίζεται καλύτερα;","why":str(c["values"])})
...
def compare_entities(db: CaseDB, case_id: int, left_id: int, right_id: int) -> dict[str, Any]:
    d=db.case_data(case_id); ents={int(e["id"]):e for e in d["entities"]}
    if left_id not in ents or right_id not in ents: raise ValueError("Και τα δύο IDs οντοτήτων πρέπει να υπάρχουν στην επιλεγμένη υπόθεση.")
    ...
    return {"left":ents[left_id],"right":ents[right_id],"classification":classification,"score":round(score,3),
            ...
            "caution":"Η συσχέτιση δεν αποδεικνύει ότι δύο λογαριασμοί, πρόσωπα, domains ή οργανισμοί αποτελούν την ίδια οντότητα ή έχουν κοινή ιδιοκτησία."}
    ...
    for e in current:
        ...
        for m in matches:
            out.append({"type":e["type"],"value":e["value"],"current_entity_id":e["id"],"other_case_id":m["case_id"],
                        ...
                        "caution":"Ο ίδιος δημόσιος δείκτης σε διαφορετικές υποθέσεις αποτελεί ένδειξη συσχέτισης και όχι αυτόματη απόδειξη κοινού actor."})
...
class InvestigationBrain:
    """Περιορισμένος event-driven planner για τεχνικά pivots δημόσιων πηγών."""
    ...
    def run(self, target: str, requested_mode: str = "auto") -> dict[str,Any]:
        ...
        while pivot_count<int(policy["max_pivots"]):
            ...
            if not heap:
                stop_reason="δεν υπάρχουν κατάλληλα pivots υψηλής αξίας"; break
...
def export_markdown(db: CaseDB, case_id: int, path: Path) -> Path:
    ...
    lines = [
        f"# Αναφορά Υπόθεσης Digital Footprint Finder — {c['name']}",
        "",
        f"- **Δημιουργήθηκε:** {c['created_at']}",
        f"- **Παράχθηκε:** {utcnow()}",
        f"- **Οντότητες:** {len(d['entities'])}",
        f"- **Σχέσεις:** {len(d['relations'])}",
        f"- **Εγγραφές αποδεικτικών:** {len(d['evidence'])}",
        "",
    ]
    if c["description"]:
        lines += ["## Περιγραφή", "", c["description"], ""]
    ...
    for e in d["entities"]:
        ...
        lines.append(
            ...
            f"(εμπιστοσύνη {e['confidence']:.2f}; πηγή: {e['source'] or 'μη καθορισμένη'})"
        )
    ...
    lines += ["", "## Αποδεικτικά στοιχεία", ""]
    for e in d["evidence"]:
        lines.append(
            f"- Αποδεικτικό `#{e['id']}` — **{e['kind']}**, καταγράφηκε {e['captured_at']}, "
            f"SHA-256 `{e['sha256']}`, bytes {e.get('body_size') or 0}"
        )
        if e["source_url"]:
            lines.append(f"  - Πηγή: {e['source_url']}")
    ...
    lines += [
        ...
        "",
        "Τα ευρήματα OSINT αποτελούν ενδείξεις και όχι αυτόματη απόδειξη ταυτότητας, ιδιοκτησίας, πρόθεσης ή παρανομίας. "
        "Διασταυρώστε σημαντικούς ισχυρισμούς με ανεξάρτητες πηγές και διατηρήστε την προέλευση των δεδομένων.",
        "",
    ]
...
def case_timeline(db: CaseDB, case_id: int, limit: int = 250) -> list[dict[str, Any]]:
    """Δημιουργεί συμπαγή χρονογραμμή από καταγεγραμμένα αποδεικτικά και ημερομηνίες δημόσιων metadata."""
    data = db.case_data(case_id)
...
def smart_case_analysis(db: CaseDB, case_id: int) -> dict[str, Any]:
    """Συνοψίζει τη διασταύρωση και ιεραρχεί χρήσιμα επόμενα pivots χωρίς ισχυρισμό ταυτότητας."""
    data = db.case_data(case_id)
    ...
    if type_counts.get("domain"):
        next_pivots.append("Ελέγξτε διασταυρωμένα hostnames, αρχειοθετημένα URLs, δημόσια έγγραφα και συνδεδεμένα profiles.")
    if type_counts.get("username") or type_counts.get("social"):
        next_pivots.append("Συγκρίνετε bios προφίλ, συνδεδεμένα domains και δημόσια metadata repositories πριν ενώσετε ταυτότητες.")
    if type_counts.get("email"):
        next_pivots.append("Συγκρίνετε το context email-domain και ανεξάρτητες δημόσιες αναφορές· μην συμπεραίνετε τρέχουσα κατοχή από μία μόνο αντιστοίχιση.")
    if type_counts.get("ip"):
        next_pivots.append("Διαχωρίστε το τρέχον DNS από ιστορικές/παθητικές παρατηρήσεις και λάβετε υπόψη shared hosting/CDNs.")
    if type_counts.get("file"):
        next_pivots.append("Επαληθεύστε timestamps και hashes των metadata με το αρχικό αρχείο· τα metadata μπορούν να τροποποιηθούν.")
    if type_counts.get("identifier") or type_counts.get("technology"):
        next_pivots.append("Συγκρίνετε επαναλαμβανόμενα δημόσια web identifiers, favicon hashes και technology fingerprints σε ανεξάρτητα παρατηρημένους ιστοτόπους.")
    if type_counts.get("asn") or type_counts.get("prefix"):
        next_pivots.append("Συγκρίνετε routing παρατηρήσεις με τρέχοντα δεδομένα DNS/RDAP· η κατοχή ASN και prefixes μπορεί να αλλάζει με τον χρόνο.")
    if type_counts.get("publication"):
        next_pivots.append("Χρησιμοποιήστε DOI/ORCID και ακριβή metadata συγγραφέων για αποσαφήνιση ακαδημαϊκών αντιστοιχίσεων ονομάτων.")
    ...
    return {
        ...
        "caution": "Η ανεξάρτητη διασταύρωση ενισχύει μια παρατήρηση· δεν αποτελεί ισχυρισμό ότι δύο εγγραφές ταυτοποιούν το ίδιο άτομο.",
    }
...
def export_html(db: CaseDB, case_id: int, path: Path) -> Path:
    ...
    timeline_html = "".join(
        ...
        for x in timeline
    ) or "<tr><td colspan='3'>Δεν υπάρχουν ακόμη δημόσιες παρατηρήσεις με ημερομηνία.</td></tr>"
    corroborated_html = "".join(
        f"<li><strong>{html.escape(str(x['type']))}</strong>: {html.escape(truncate(str(x['value']), 120))} "
        f"— {len(x['sources'])} πηγές, εμπιστοσύνη {x['confidence']:.2f}</li>"
        for x in analysis["corroborated"][:12]
    ) or "<li>Δεν υπάρχει ακόμη διασταύρωση από πολλαπλές πηγές.</li>"
    pivots_html = "".join(f"<li>{html.escape(x)}</li>" for x in analysis["next_pivots"]) or "<li>Επαληθεύστε χειροκίνητα τις ισχυρότερες ενδείξεις.</li>"
    questions_html = "".join(
    ...
    ) or "<li>Δεν δημιουργήθηκαν αναπάντητες ερωτήσεις υψηλής προτεραιότητας.</li>"
    contradictions_html = "".join(
        f"<li>Entity #{x.get('entity_id')}: {html.escape(str(x.get('note','')))}</li>" for x in contradictions
    ) or "<li>Δεν εντοπίστηκαν ρητές αντιφάσεις από τους τρέχοντες συντηρητικούς κανόνες.</li>"
    central_html = "".join(
        f"<li>#{x['id']} <strong>{html.escape(str(x['type']))}</strong>: {html.escape(truncate(str(x['value']),100))} — βαθμός {x['degree']}</li>"
        for x in graph_info.get("central_entities",[])[:10]
    ) or "<li>Δεν υπάρχουν ακόμη κεντρικοί κόμβοι στον γράφο.</li>"
    ...
    evidence_rows = "\n".join(
        ...
        f"<td>{('<a href=\"' + html.escape(e['source_url'], quote=True) + '\">πηγή</a>') if e['source_url'] else ''}</td>"
        "</tr>"
        for e in d["evidence"]
    )
...
<body>
<header>
<h1>Αναφορά Υπόθεσης Digital Footprint Finder</h1>
<h2>{html.escape(c['name'])}</h2>
<p>{html.escape(c['description'])}</p>
<p class="small">Παράχθηκε {utcnow()} · {len(d['entities'])} οντότητες ·
{len(d['relations'])} σχέσεις · {len(d['evidence'])} εγγραφές αποδεικτικών</p>
</header>

<section class="card">
<h2>Έξυπνη Ανάλυση</h2>
<p class="small">Το Digital Footprint Finder συγχωνεύει διπλότυπες οντότητες και αυξάνει την εμπιστοσύνη μόνο όταν συμφωνούν ανεξάρτητες δημόσιες παρατηρήσεις.</p>
<h3>Διασταυρωμένες παρατηρήσεις</h3><ul>{corroborated_html}</ul>
<h3>Προτεινόμενα επόμενα pivots</h3><ul>{pivots_html}</ul>
<p class="small">{html.escape(analysis['caution'])}</p>
</section>

<section class="card">
<h2>Εστίαση Ερευνητή</h2>
<p><strong>Ποιότητα έρευνας:</strong> {quality['score']}/100 — {html.escape(str(quality['label']))}</p>
<p class="small">{html.escape(str(quality['note']))}</p>
<h3>Ιεραρχημένες αναπάντητες ερωτήσεις</h3><ol>{questions_html}</ol>
<h3>Πιθανές αντιφάσεις</h3><ul>{contradictions_html}</ul>
<h3>Κεντρικές οντότητες γράφου</h3><ul>{central_html}</ul>
</section>

<section class="card">
<h2>Χρονογραμμή Έρευνας</h2>
<p class="small">Οι ημερομηνίες εξάγονται από καταγεγραμμένα δημόσια metadata και archive indexes. Μπορεί να περιγράφουν διαφορετικά γεγονότα και πρέπει να ερμηνεύονται στο σωστό context.</p>
<table><thead><tr><th>Ώρα UTC</th><th>Τύπος</th><th>Παρατήρηση</th></tr></thead><tbody>{timeline_html}</tbody></table>
</section>

<section class="card">
<h2>Χάρτης Σχέσεων</h2>
<p class="small">Εμφανίζονται έως 80 οντότητες. Οι γραμμές δείχνουν καταγεγραμμένες σχέσεις.</p>
...
<section class="card"><h2>Οντότητες</h2>
<table><thead><tr><th>ID</th><th>Τύπος</th><th>Τιμή</th><th>Εμπιστοσύνη</th><th>Πηγή</th></tr></thead>
<tbody>{entity_rows}</tbody></table></section>

<section class="card"><h2>Σχέσεις</h2>
<table><thead><tr><th>Από</th><th>Σχέση</th><th>Προς</th><th>Εμπιστοσύνη</th></tr></thead>
<tbody>{relation_rows}</tbody></table></section>

<section class="card"><h2>Αλυσίδα Αποδεικτικών</h2>
<table><thead><tr><th>ID</th><th>Τύπος</th><th>Καταγραφή</th><th>SHA-256</th><th>Πηγή</th></tr></thead>
<tbody>{evidence_rows}</tbody></table></section>

<section class="card"><h2>Σημείωση Επαλήθευσης</h2>
<p>Τα ευρήματα OSINT αποτελούν ενδείξεις και όχι αυτόματη απόδειξη ταυτότητας, ιδιοκτησίας, πρόθεσης ή παρανομίας.
Διασταυρώστε σημαντικούς ισχυρισμούς ανεξάρτητα και διατηρήστε την προέλευση των δεδομένων.</p>
</section>
</body></html>"""
...
def export_raw_evidence(db: CaseDB, case_id: int, out_dir: Path) -> list[Path]:
    """Υλοποιεί καταγεγραμμένα response bodies και sidecar metadata για δυνατότητα audit."""
...
def build_case_package(db: CaseDB, case_id: int, out_dir: Path, files: list[Path]) -> Path:
    ...
    manifest_lines = ["Πακέτο αποδεικτικών Digital Footprint Finder", f"Παράχθηκε: {utcnow()}", ""]
    for f in files:
...
# ---------------------------------------------------------------------------
# Ελληνική παρουσίαση δομημένων αποτελεσμάτων
# ---------------------------------------------------------------------------
# Οι εσωτερικοί keys παραμένουν σταθεροί για συμβατότητα. Η μετάφραση γίνεται
# μόνο στην οθόνη, ώστε JSON/STIX/GraphML exports και αποθηκευμένες υποθέσεις να
# παραμένουν διαλειτουργικά.
DISPLAY_KEY_TRANSLATIONS = {
    "case": "υπόθεση", "description": "περιγραφή", "entities": "οντότητες",
    "relations": "σχέσεις", "evidence": "αποδεικτικά", "notes": "σημειώσεις",
    "assessments": "αξιολογήσεις", "hypotheses": "υποθέσεις_εργασίας", "events": "γεγονότα",
    "runs": "εκτελέσεις", "id": "id", "type": "τύπος", "value": "τιμή",
    "label": "ετικέτα", "confidence": "εμπιστοσύνη", "source": "πηγή",
    "created_at": "δημιουργήθηκε", "relation": "σχέση", "captured_at": "καταγράφηκε",
    "content_type": "τύπος_περιεχομένου", "metadata": "metadata", "body_size": "μέγεθος_body",
    "status": "κατάσταση", "target": "στόχος", "result": "αποτέλεσμα",
    "errors": "σφάλματα", "warning": "προειδοποίηση", "note": "σημείωση",
    "caution": "προσοχή", "question": "ερώτηση", "why": "γιατί", "priority": "προτεραιότητα",
    "score": "βαθμολογία", "quality": "ποιότητα", "strongest": "ισχυρότερα",
    "corroborated": "διασταυρωμένα", "next_pivots": "επόμενα_pivots",
    "source_coverage": "κάλυψη_πηγών", "timeline_events": "γεγονότα_χρονογραμμής",
    "central_entities": "κεντρικές_οντότητες", "contradictions": "αντιφάσεις",
    "independent_sources": "ανεξάρτητες_πηγές", "source_families": "οικογένειες_πηγών",
    "evidence_records": "εγγραφές_αποδεικτικών", "human_assessments": "ανθρώπινες_αξιολογήσεις",
    "unresolved_contradictions": "ανεπίλυτες_αντιφάσεις", "incoming": "εισερχόμενες_σχέσεις",
    "outgoing": "εξερχόμενες_σχέσεις", "recommended_checks": "προτεινόμενοι_έλεγχοι",
    "why_it_matters": "γιατί_έχει_σημασία", "configured": "ρυθμισμένο",
    "optional_credentials": "προαιρετικά_credentials", "keyless_public_sources": "δημόσιες_πηγές_χωρίς_κλειδί",
    "privacy": "ιδιωτικότητα", "purpose": "σκοπός", "name": "όνομα",
    "provider_hints": "ενδείξεις_παρόχου", "search_links": "σύνδεσμοι_αναζήτησης",
    "public_emails": "δημόσια_emails", "public_phones": "δημόσια_τηλέφωνα",
    "social_links": "social_σύνδεσμοι", "documents": "έγγραφα", "javascript": "javascript",
    "tracker_ids": "δημόσια_identifiers", "resolved_ips": "επιλυμένες_IPs",
    "certificates": "πιστοποιητικά", "wayback": "wayback", "crawl": "crawling",
    "reverse_dns": "reverse_dns", "ip_intelligence": "πληροφορίες_IP",
    "probable_profiles": "πιθανά_profiles", "possible_profiles": "ενδεχόμενα_profiles",
    "alias_variants": "παραλλαγές_alias", "algorithm": "αλγόριθμος", "hash": "hash",
    "size": "μέγεθος", "title": "τίτλος", "publisher": "εκδότης", "author": "συγγραφέας",
    "answer": "απάντηση", "focus": "εστίαση",
}

DISPLAY_VALUE_TRANSLATIONS = {
    "available": "διαθέσιμο", "configured": "ρυθμισμένο", "not configured": "μη ρυθμισμένο",
    "environment": "μεταβλητή περιβάλλοντος", "secure local file": "ασφαλές τοπικό αρχείο",
    "probable": "πιθανό", "possible": "ενδεχόμενο", "unknown": "άγνωστο",
    "found": "βρέθηκε", "not found": "δεν βρέθηκε", "none found": "δεν βρέθηκε κάτι",
    "early": "αρχικό", "developing": "σε ανάπτυξη", "solid": "ισχυρό", "well-supported": "πολύ καλά τεκμηριωμένο",
    "confirmed": "επιβεβαιωμένο", "rejected": "απορριφθέν", "uncertain": "αβέβαιο",
    "likely-related": "πιθανώς σχετιζόμενα", "insufficient": "ανεπαρκή στοιχεία",
    "saved": "αποθηκεύτηκε", "review": "απαιτεί έλεγχο",
}

def localize_for_display(value: Any) -> Any:
    """Μεταφράζει μόνο την παρουσίαση χωρίς να μεταβάλλει τα εσωτερικά δεδομένα."""
    if isinstance(value, dict):
        return {DISPLAY_KEY_TRANSLATIONS.get(str(k), str(k)): localize_for_display(v) for k, v in value.items()}
    if isinstance(value, list):
        return [localize_for_display(v) for v in value]
    if isinstance(value, tuple):
        return [localize_for_display(v) for v in value]
    if isinstance(value, str):
        return DISPLAY_VALUE_TRANSLATIONS.get(value, value)
    return value
...
def print_result(data: Any) -> None:
    print(pretty_json(localize_for_display(data)))


def print_case_summary(db: CaseDB, case_id: int) -> None:
    s = case_summary(db, case_id)
    UI.title(f"Υπόθεση: {s['case']}")
    print(f"  Οντότητες     {s['entities']}")
    print(f"  Σχέσεις       {s['relations']}")
    print(f"  Αποδεικτικά   {s['evidence']}")
    print(f"  Σημειώσεις    {s['notes']}")
    print(f"  Πηγές         {s.get('independent_sources', 0)}")
    print(f"  Χρονογραμμή   {s.get('timeline_events', 0)} γεγονότα")
    if s["entity_types"]:
        top = ", ".join(f"{k}: {v}" for k, v in list(s["entity_types"].items())[:8])
        print(f"  Βρέθηκαν       {top}")
...
def print_easy_result(payload: dict[str, Any]) -> None:
    ...
    UI.ok(f"Ανιχνεύτηκε {typ}: {target}")
    ...
    if typ == "domain":
        ...
        print(f"  Διευθύνσεις IP:     {', '.join(ips[:6]) if ips else 'none found'}")
        print(f"  Ονόματα πιστοποιητ.:{len(certs)}")
        print(f"  Αρχειοθετημένα URLs:{len(wb)}")
        if web.get("status"):
            print(f"  Ιστότοπος:          HTTP {web.get('status')} — {web.get('title') or 'χωρίς τίτλο'}")
        ...
        print(f"  Παθητικές παρατηρήσεις:{passive_count:>3}")
        ...
        if mail:
            ...
            print(f"  Προστασίες email:   {', '.join(enabled) if enabled else 'δεν εντοπίστηκαν'}")
        if crawl:
            print(f"  Σελίδες που ελέγχθηκαν:{len(crawl.get('pages', []))}")
            print(f"  Δημόσια έγγραφα:    {len(crawl.get('documents', []))}")
            print(f"  JS endpoints:        {len(crawl.get('endpoints', []))}")
    elif typ == "url":
        print(f"  Κατάσταση HTTP:      {result.get('status')}")
        print(f"  Τελικό URL:          {result.get('final_url')}")
        print(f"  Τίτλος σελίδας:      {result.get('title') or 'κανένας'}")
        pi = result.get("page_intelligence", {})
        print(f"  Δημόσια emails:      {len(pi.get('public_emails', []))}")
        print(f"  Δημόσια τηλέφωνα:    {len(pi.get('public_phones', []))}")
        print(f"  Σύνδεσμοι που βρέθηκαν:{len(result.get('links', []))}")
        technologies = (result.get("technology_hints") or {}).get("technologies", [])
        if technologies:
            print(f"  Ενδείξεις τεχνολογίας:{', '.join(str(x.get('name')) for x in technologies[:8])}")
        pi = result.get("page_intelligence", {})
        if pi.get("tracker_ids"):
            print(f"  Δημόσια identifiers: {len(pi.get('tracker_ids', []))}")
        if result.get("favicon_hashes"):
            print(f"  Favicon hashes:      {len(result.get('favicon_hashes', []))}")
        if result.get("document_metadata"):
            print(f"  Metadata εγγράφου:   βρέθηκαν")
        crawl = result.get("crawl") or {}
        if crawl:
            print(f"  Σελίδες που ελέγχθηκαν:{len(crawl.get('pages', []))}")
            print(f"  Δημόσια έγγραφα:    {len(crawl.get('documents', []))}")
        ...
        possible = [r for r in rows if r.get("result") == "possible"]
        unknown = [r for r in rows if r.get("result") == "unknown"]
        print(f"  Ιστότοποι ελέγχθηκαν:{len(rows)}")
        print(f"  Πιθανά profiles:     {len(probable)}")
        print(f"  Ενδεχόμενα profiles: {len(possible)}")
        for row in (probable + possible)[:12]:
            print(f"    • {row.get('site')}: {row.get('url')} [{row.get('confidence', 0):.2f}]")
        if unknown:
            print(f"  Αβέβαιοι έλεγχοι:    {len(unknown)}")
        print(f"  GitHub API:         {'βρέθηκε δημόσιο profile' if result.get('github_api') else 'δεν βρέθηκε ακριβές δημόσιο profile'}")
        print(f"  GitLab API:         {len(result.get('gitlab_api', []))} ακριβές/ή δημόσιο/α αποτέλεσμα/τα")
        print(f"  Προτάσεις aliases:   {len(result.get('alias_variants', []))}")
        print(f"  Ενδείξεις dorks:     {len((result.get('dorks') or {}).get('links', []))}")
        UI.warn("Οι αντιστοιχίσεις URL προφίλ είναι ενδείξεις και όχι απόδειξη ότι οι λογαριασμοί ανήκουν στο ίδιο άτομο.")
    elif typ == "email":
        ...
        if providers:
            print(f"  Ένδειξη παρόχου mail:{', '.join(providers)}")
        print(f"  Ενδείξεις αναζήτησης:{len(result.get('search_links', []))}")
    elif typ == "ip":
        print(f"  Reverse DNS:         {', '.join(result.get('reverse_dns', [])) or 'none found'}")
        print(f"  Δεδομένα RDAP:       {'βρέθηκαν' if result.get('rdap') else 'δεν βρέθηκαν'}")
        geo = result.get("ip_intelligence") or {}
        if geo:
            print(f"  Κατά προσέγγιση τοποθεσία:{', '.join(str(x) for x in [geo.get('city'), geo.get('region'), geo.get('country')] if x) or 'άγνωστο'}")
    elif typ in {"person", "phone", "organization"}:
        links = result.get("search_links", [])
        print(f"  Δημόσιες ενδείξεις αναζήτησης:{len(links):>4}")
        for row in links[:6]:
            print(f"    • {row.get('engine')}: {row.get('url')}")
        if typ == "person":
            print(f"  Υποψήφια Wikidata:  {len(result.get('wikidata_candidates', [])):>3}")
            print(f"  Ακαδημαϊκά υποψήφια:{len(result.get('crossref_candidates', [])):>2}")
        elif typ == "organization":
            print(f"  Υποψήφια Wikidata:  {len(result.get('wikidata_candidates', [])):>3}")
        UI.warn("Οι υποψήφιες αντιστοιχίσεις/αναζητήσεις πρέπει να διασταυρώνονται πριν τις συνδέσετε με πραγματικό πρόσωπο ή οργανισμό.")
    elif typ == "doi":
        meta = result.get("metadata") or {}
        print(f"  DOI:                {result.get('doi')}")
        print(f"  Τίτλος:              {meta.get('title') or 'not found'}")
        print(f"  Εκδότης:             {meta.get('publisher') or 'άγνωστο'}")
        print(f"  Συγγραφείς:          {len(meta.get('author', []))}")
    elif typ == "asn":
        print(f"  ASN:                {result.get('asn')}")
        print(f"  Κάτοχος:             {(result.get('overview') or {}).get('holder') or 'άγνωστο'}")
        print(f"  Ανακοινωμένα prefixes:{len(result.get('prefixes', []))}")
    elif typ == "prefix":
        print(f"  Prefix:              {result.get('prefix')}")
        overview = result.get('overview') or {}
        print(f"  ASNs ανακοίνωσης:    {', '.join('AS'+str(x) if not str(x).upper().startswith('AS') else str(x) for x in overview.get('asns', [])[:8]) or 'none found'}")
    elif typ == "hash":
        print(f"  Αλγόριθμος:          {result.get('algorithm')}")
        print(f"  Hash:               {result.get('hash')}")
    elif typ == "file":
        print(f"  Αρχείο:              {result.get('name')}")
        print(f"  Μέγεθος:             {human_size(int(result.get('size', 0)))}")
        print(f"  SHA-256:            {result.get('sha256')}")
        if result.get("exif"):
            print(f"  Πεδία EXIF:          {len(result['exif'])}")

    errors = result.get("errors", []) if isinstance(result, dict) else []
    if errors:
        UI.warn(f"{len(errors)} πηγή/ές δεν ήταν δυνατό να ερωτηθούν. Η υπόλοιπη υπόθεση αποθηκεύτηκε κανονικά.")
...
def run_easy_investigation(db: CaseDB, target: str, case_name: Optional[str] = None,
    ...
    case_id = db.get_or_create_case(name, f"Καθοδηγούμενη έρευνα για {target}")
    ...
    UI.info(f"Έρευνα για {target} · λειτουργία: {effective_mode}")
    ...
    if analysis.get("corroborated"):
        UI.ok(f"Διασταύρωση μεταξύ πηγών: {len(analysis['corroborated'])} παρατήρηση/εις")
    ...
    if html_paths:
        UI.ok(f"Αναγνώσιμη αναφορά: {html_paths[0]}")
        if settings.get("auto_open_report"):
            open_local_file(html_paths[0])
    if zip_paths:
        UI.ok(f"Πακέτο αποδεικτικών: {zip_paths[0]}")
    return case_id, payload, paths

def print_focus(db: CaseDB, case_id: int) -> None:
    f=focus_summary(db,case_id); q=f["quality"]
    UI.title("Τι έχει σημασία τώρα")
    print(f"  Ποιότητα έρευνας       {q['score']}/100 · {DISPLAY_VALUE_TRANSLATIONS.get(str(q['label']), q['label'])}")
    print(f"  Γράφος αποδεικτικών    {f['entities']} οντότητες · {f['relations']} σχέσεις · {f['evidence']} αποδεικτικά")
    print(f"  Αντιφάσεις             {len(f['contradictions'])}")
    if f["strongest"]:
        print("\n  Ισχυρότερα τεκμηριωμένα ευρήματα:")
        for e in f["strongest"][:5]:
            print(f"    #{e['id']} {e['type']}: {truncate(str(e['value']),72)} · {len(e['source_families'])} οικογένειες πηγών · {float(e['confidence']):.2f}")
    if f["questions"]:
        best=f["questions"][0]
        print(f"\n  Καλύτερη επόμενη ερώτηση:\n    {best['question']}\n    Γιατί: {best['why']}")


def render_search_results(rows: list[dict[str,Any]]) -> None:
    if not rows:
        UI.warn("Δεν βρέθηκαν αποθηκευμένα ευρήματα που να ταιριάζουν.")
        return
    UI.title("Αποτελέσματα αναζήτησης")
    for i,r in enumerate(rows,1):
        kind=r.get("kind")
        if kind=="entity":
            print(f"[{i}] [οντότητα] #{r['id']} {r['type']}: {truncate(str(r['value']),90)}")
            print(f"    υπόθεση: {r.get('case_name')} · εμπιστοσύνη {float(r.get('confidence',0)):.2f} · {truncate(str(r.get('source') or ''),100)}")
        elif kind=="evidence":
            print(f"[{i}] [αποδεικτικό] #{r['id']} {r.get('evidence_kind')} · {truncate(str(r.get('source_url') or ''),100)}")
        elif kind=="note":
            print(f"[{i}] [σημείωση] #{r['id']} {truncate(str(r.get('text') or ''),110)}")
        else:
...
def settings_set_value(settings: dict[str,Any], key: str, raw: str) -> Any:
    if key not in DEFAULT_SETTINGS: raise ValueError(f"Άγνωστη ρύθμιση: {key}")
    current=DEFAULT_SETTINGS[key]
    if isinstance(current,bool):
        v=raw.lower()
        if v not in {"on","off","true","false","yes","no","1","0"}: raise ValueError("Χρησιμοποιήστε on/off.")
    ...
    if isinstance(current,int):
        ...
        if key in ranges and not (ranges[key][0]<=v<=ranges[key][1]): raise ValueError(f"Το {key} πρέπει να είναι {ranges[key][0]}–{ranges[key][1]}.")
        return v
    if key=="default_mode" and raw not in {"auto","passive","normal","deep"}: raise ValueError("Χρησιμοποιήστε auto/passive/normal/deep.")
    if key=="search_scope" and raw not in {"current","all"}: raise ValueError("Χρησιμοποιήστε current/all.")
    if key=="colors" and raw not in {"auto","on","off"}: raise ValueError("Χρησιμοποιήστε auto/on/off.")
    return raw
...
def settings_menu() -> None:
    settings=load_settings()
    labels=[
        ("default_mode","Προεπιλεγμένη λειτουργία έρευνας"),("network_timeout","Χρονικό όριο δικτύου"),
        ("guided_followup","Μενού καθοδηγούμενης συνέχειας"),("auto_open_report","Αυτόματο άνοιγμα HTML αναφοράς"),
        ("auto_install_dependencies","Αυτόματη εγκατάσταση προαιρετικών Python enrichers"),("search_scope","Προεπιλεγμένο εύρος αναζήτησης"),
        ("search_limit","Όριο αποτελεσμάτων αναζήτησης"),("colors","Χρώματα τερματικού"),("resume_latest_case","Συνέχιση τελευταίας υπόθεσης"),
        ("max_crawl_pages","Προεπιλεγμένο όριο σελίδων crawling"),("username_workers","Workers ελέγχου username"),
        ("json_cache_ttl","TTL δημόσιου JSON cache (δευτερόλεπτα)"),("save_raw_evidence","Αποθήκευση ακατέργαστων HTTP αποδεικτικών"),
    ]
    while True:
        UI.title("Ρυθμίσεις")
        for i,(key,label) in enumerate(labels,1): print(f"[{i}] {label:38} {settings.get(key)}")
        print("[A] Προαιρετικά API credentials")
        print("[0] Πίσω")
        raw=input("Επιλέξτε ρύθμιση: ").strip()
        if raw in {"","0"}: break
        ...
        try:
            ...
            save_settings(settings); apply_color_setting(settings); UI.ok("Αποθηκεύτηκε.")
...
def local_search_prompt(db: CaseDB, case_id: Optional[int]=None, initial: str="") -> None:
    settings=load_settings(); q=initial.strip() or input("Αναζήτηση στα αποθηκευμένα ευρήματα: ").strip()
    if not q: return
    ...
    if rows:
        choice=input("Αριθμός αποτελέσματος οντότητας για άνοιγμα [Enter = τέλος]: ").strip()
        if choice.isdigit():
            i=int(choice)-1
...
def ask_case(db: CaseDB, case_id: int, question: str) -> Any:
    ...
    if any(x in q for x in ("what next", "check next", "next step", "question", "τι μετά", "τι μετα", "επόμενο βήμα", "επομενο βημα", "τι να ελέγξω", "τι να ελεγξω", "ερώτηση", "ερωτηση")):
        return ranked_questions(db,case_id,10)
    if any(x in q for x in ("strong", "matter", "summary", "ισχυρ", "σημαντ", "σύνοψη", "συνοψη")):
        return focus_summary(db,case_id)
    if any(x in q for x in ("contradiction", "conflict", "αντίφαση", "αντιφαση", "σύγκρουση", "συγκρουση")):
        return case_contradictions(db,case_id)
    if "quality" in q or "ποιότητα" in q or "ποιοτητα" in q:
        return investigation_quality(db,case_id)
    if any(x in q for x in ("connect", "συνδέ", "συνδε")) and len(ids)>=2:
        return relationship_path(db,case_id,ids[0],ids[1])
    if any(x in q for x in ("compare", "σύγκρινε", "συγκρινε")) and len(ids)>=2:
        return compare_entities(db,case_id,ids[0],ids[1])
    if any(x in q for x in ("cluster", "related sites", "συστάδα", "συσταδα", "σχετικοί ιστότοποι", "σχετικοι ιστοτοποι")):
        return correlation_clusters(db,case_id)
    if any(x in q for x in ("explain", "εξήγησε", "εξηγησε")) and ids:
        return explain_entity(db,case_id,ids[0])
    return {"answer":"Μπορώ να απαντήσω: τι μετά, ισχυρότερα ευρήματα, αντιφάσεις, ποιότητα, εξήγησε <id οντότητας> ή τι συνδέει <id> και <id>.",
            "focus":focus_summary(db,case_id)}


def home_menu(db: CaseDB) -> str:
    UI.title("Αρχική")
    print("[1] Έρευνα στοιχείου\n[2] Αναζήτηση αποθηκευμένων ευρημάτων\n[3] Συνέχεια τρέχουσας/πρόσφατης υπόθεσης\n[4] Ερώτηση για την τρέχουσα υπόθεση\n[5] Υποθέσεις\n[6] Ρυθμίσεις\n[7] Βοήθεια\n[0] Έξοδος")
...
def guided_followup(db: CaseDB, case_id: int, paths: list[Path]) -> None:
    ...
    while True:
        ...
        print("\n[Enter] Τέλος  [1] Βαθύτερη συνέχεια  [2] Αναζήτηση υπόθεσης  [3] Εξήγηση οντότητας  [4] Αξιολόγηση οντότητας")
        print("[5] Προσθήκη στοιχείου  [6] Άνοιγμα αναφοράς  [7] Ανανέωση exports  [8] Ρυθμίσεις  [9] Ερώτηση υπόθεσης")
        c=input("Ενέργεια: ").strip()
        if not c: return
        try:
            if c=="1":
                session=load_session(); target=str(session.get("latest_target") or "").strip() or input("Στόχος για συνέχεια: ").strip()
                if target:
                    UI.info("Συνέχεια με περιορισμένη λειτουργία Deep …")
                ...
                raw=input("ID οντότητας: ").strip().lstrip("#"); print_result(explain_entity(db,case_id,int(raw)))
            elif c=="4":
                eid=int(input("ID οντότητας: ").strip().lstrip("#")); status=input("confirmed / rejected / uncertain: ").strip().lower(); note=input("Σημείωση (προαιρετική): ")
                db.add_assessment(case_id,eid,status,note); UI.ok("Η αξιολόγηση αποθηκεύτηκε.")
            elif c=="5":
                target=input("Επιπλέον στοιχείο: ").strip()
                if target:
                ...
                if report and open_local_file(report): UI.ok("Η αναφορά άνοιξε.")
                else: UI.warn("Δεν ήταν δυνατό να ανοίξει αυτόματα· η διαδρομή της αναφοράς εμφανίζεται στον φάκελο της υπόθεσης.")
            elif c=="7": paths[:]=export_all(db,case_id,case_files_dir(db.get_case(case_id)["name"])); UI.ok("Τα exports ανανεώθηκαν.")
            elif c=="8": settings_menu()
            elif c=="9": print_result(ask_case(db,case_id,input("Ρωτήστε: ")))
            else: UI.warn("Άγνωστη ενέργεια.")
...
def choose_existing_case(db: CaseDB) -> Optional[int]:
    ...
    if not cases:
        UI.warn("Δεν υπάρχουν ακόμη αποθηκευμένες υποθέσεις.")
        return None
    UI.title("Πρόσφατες υποθέσεις")
    for i, row in enumerate(cases, 1):
        print(f"[{i}] {row['name']}  ({row['created_at']})")
    raw = input("Επιλέξτε αριθμό υπόθεσης [Enter = ακύρωση]: ").strip()
    if not raw:
        return None
    try:
        idx = int(raw) - 1
        ...
        UI.err("Μη έγκυρος αριθμός υπόθεσης.")
        return None
...
Προχωρημένα εργαλεία
 ...
 [4] Email         [5] IP           [6] Πρόσωπο
 [7] Τηλέφωνο      [8] Τοπικό αρχείο [9] Σύνδεσμοι αναζήτησης
 [10] Πακέτο dorks  [11] Προσθήκη σημείωσης [12] Raw JSON
 [13] Export       [14] ASN          [15] CIDR prefix
 [16] DOI          [17] Οργανισμός   [18] Χρονογραμμή
 [0] Πίσω
""")
        choice = input("προχωρημένα> ").strip()
        try:
            ...
            elif choice == "6": print_result(det.person(input("Πλήρες όνομα: ")))
            elif choice == "7": print_result(det.phone(input("Τηλέφωνο: ")))
            elif choice == "8": print_result(det.local_file(input("Διαδρομή αρχείου: ")))
            elif choice == "9": print_result(det.search_links([input("Ερώτημα αναζήτησης: ")]))
            elif choice == "10":
                target = input("Στόχος για dorks: ")
                root = det._entity(detect_target_type(target), target, confidence=1.0, source="user")
                print_result(det.dorks(target, attach_to=root))
            elif choice == "11":
                db.add_note(case_id, input("Σημείωση υπόθεσης: "))
                UI.ok("Η σημείωση αποθηκεύτηκε.")
            ...
            elif choice == "17": print_result(det.organization(input("Οργανισμός: ")))
            elif choice == "18": print_result(case_timeline(db, case_id))
            elif choice == "0": return
            else: UI.err("Άγνωστη επιλογή.")
...
def doctor() -> dict[str, Any]:
    ...
    checks: dict[str, Any] = {
        ...
        "dig": shutil.which("dig") or "δεν είναι εγκατεστημένο (ενεργό DoH fallback)",
        "database_write": False,
        "api_sources": api_status_report(),
    }
    for module in BOOTSTRAP_PACKAGES:
        checks[module] = "available" if _module_available(module) else "λείπει (αυτόματη εγκατάσταση κατά την εκκίνηση)"
    try:
...
ΕΥΚΟΛΟΤΕΡΟΣ ΤΡΟΠΟΣ
  python "Digital Footprint Finder Greek.py"
  Επικολλήστε ένα στοιχείο. Πατήστε Enter για την Αρχική.
...
ΠΑΡΑΔΕΙΓΜΑΤΑ
  ...
  @alice@example.social       ρητό Fediverse handle
  8.8.8.8                     IP
  +30 210 123 4567            τηλέφωνο
  Jane Example                πρόσωπο/όνομα
  org:Example Foundation      οργανισμός
  AS13335                     autonomous system
  1.1.1.0/24                  network prefix
  10.1000/example             DOI
  <64 hex χαρακτήρες>         SHA-256 hash
  /path/to/photo.jpg          τοπικό αρχείο

ΔΙΑΔΡΑΣΤΙΚΕΣ ΕΝΤΟΛΕΣ
  search for example.com      αναζήτηση σε αποθηκευμένα δεδομένα
  find alice                  εύρεση σε αποθηκευμένα δεδομένα
  ask what should I check next?
  τι μετά;                    ελληνική ερώτηση για επόμενο βήμα
...
ΛΕΙΤΟΥΡΓΙΕΣ ΕΡΕΥΝΑΣ
  auto      συντηρητική επιλογή για αμφίσημους στόχους ταυτότητας και normal για δομημένους τεχνικούς στόχους
  passive   μόνο η αρχική έρευνα δημόσιων πηγών
  normal    περιορισμένα αναδρομικά pivots υψηλής αξίας
  deep      μεγαλύτερος αλλά πάντα περιορισμένος προϋπολογισμός παθητικών/δημόσιων pivots

ΠΑΡΑΔΕΙΓΜΑΤΑ CLI
  python "Digital Footprint Finder Greek.py" --mode normal example.com
  python "Digital Footprint Finder Greek.py" fediverse @alice@example.social
  python "Digital Footprint Finder Greek.py" --case "Acme" search example.com
  python "Digital Footprint Finder Greek.py" --case "Acme" ask "what should I check next?"
  python "Digital Footprint Finder Greek.py" --case "Acme" explain 17
  python "Digital Footprint Finder Greek.py" --case "Acme" path 17 42
  python "Digital Footprint Finder Greek.py" --case "Acme" compare 17 42
  python "Digital Footprint Finder Greek.py" --case "Acme" clusters
  python "Digital Footprint Finder Greek.py" --case "Acme" crosscase
  python "Digital Footprint Finder Greek.py" --case "Acme" audit
  python "Digital Footprint Finder Greek.py" --case "Acme" quality
  python "Digital Footprint Finder Greek.py" --case "Acme" questions
  python "Digital Footprint Finder Greek.py" --case "Acme" contradictions
  python "Digital Footprint Finder Greek.py" settings
  python "Digital Footprint Finder Greek.py" settings default_mode deep
  python "Digital Footprint Finder Greek.py" api list
  python "Digital Footprint Finder Greek.py" api set github
  python "Digital Footprint Finder Greek.py" api remove github
  python "Digital Footprint Finder Greek.py" websearch "example.com contact"
...
ΣΗΜΑΝΤΙΚΟ
  ...
  Υποψήφια πρόσωπα/οργανισμοί παραμένουν ξεχωριστά μέχρι να διασταυρωθούν ανεξάρτητα.
  Το Digital Footprint Finder χρησιμοποιεί δημόσιες πηγές και περιορισμένο crawling· δεν παρακάμπτει authentication.
  Η λογική OSINT είναι εγγενής σε αυτό το αρχείο· δεν κάνει clone ούτε εκτελεί εξωτερικά OSINT projects.
""")
...
def interactive(db_path: Path) -> None:
    ...
    print("Επικολλήστε ένα στοιχείο ή πατήστε Enter για την Αρχική. Το Digital Footprint Finder επιλέγει αυτόματα ασφαλή λειτουργία έρευνας.")
    print("Εντολές: search  settings  ask  cases  open  help  doctor  advanced  quit\nΕλληνικά aliases: αναζήτηση  ρυθμίσεις  ρώτα  υποθέσεις  άνοιγμα  βοήθεια  έλεγχος  προχωρημένα  έξοδος\n")
    db=CaseDB(db_path)
    try:
        while True:
            try: raw=input(UI.style("Έρευνα / εντολή\n> ","bold")).strip()
            except EOFError: print(); break
            except KeyboardInterrupt: print("\nΠληκτρολογήστε quit ή έξοδος για τερματισμό."); continue
            if not raw:
                choice=home_menu(db)
                if choice=="0": break
                if choice=="1": raw=input("Στοιχείο: ").strip()
                    ...
                    else: UI.warn("Δεν υπάρχει ακόμη αποθηκευμένη υπόθεση.")
                    continue
                elif choice=="4":
                    cid=current_case_id(db)
                    if cid: print_result(ask_case(db,cid,input("Ρωτήστε: ")))
                    else: UI.warn("Δεν υπάρχει ακόμη αποθηκευμένη υπόθεση.")
                    continue
            ...
            greek_exact = {
                "έξοδος":"quit", "εξοδος":"quit", "κλείσιμο":"quit", "κλεισιμο":"quit",
                "βοήθεια":"help", "βοηθεια":"help", "ρυθμίσεις":"settings", "ρυθμισεις":"settings",
                "υποθέσεις":"cases", "υποθεσεις":"cases", "άνοιγμα":"open", "ανοιγμα":"open",
                "έλεγχος":"doctor", "ελεγχος":"doctor", "προχωρημένα":"advanced", "προχωρημενα":"advanced",
                "api κλειδιά":"api", "api κλειδια":"api",
            }
            command = greek_exact.get(command, command)
            for gr,en in (("αναζήτηση για ","search for "),("αναζητηση για ","search for "),("αναζήτηση ","search "),("αναζητηση ","search "),("βρες ","find "),("ρώτα ","ask "),("ρωτα ","ask "),("νέα ","new "),("νεα ","new ")):
                if command.startswith(gr):
                    command=en+command[len(gr):]
                    raw=en+raw[len(gr):]
                    break
            ...
            if command=="cases":
                rows=db.list_cases()[:25]; UI.title("Πρόσφατες υποθέσεις")
                if not rows: UI.warn("Δεν υπάρχουν ακόμη αποθηκευμένες υποθέσεις.")
            ...
            if command=="open":
                ...
                continue
            if command=="advanced":
                cid=current_case_id(db) or db.get_or_create_case("Χειροκίνητη Έρευνα "+dt.datetime.now().strftime("%Y-%m-%d %H%M")); advanced_menu(db,cid); continue
            ...
            if command.startswith("ask "):
                ...
                continue
            if command in {"what next?","what next","what should i check next?","show strongest findings","show contradictions","show quality",
                           "τι μετά;","τι μετα;","τι μετά","τι μετα","ισχυρότερα ευρήματα","ισχυροτερα ευρηματα",
                           "δείξε αντιφάσεις","δειξε αντιφασεις","δείξε ποιότητα","δειξε ποιοτητα"} \
                    or command.startswith(("explain ","what connects ","εξήγησε ","εξηγησε ","τι συνδέει ","τι συνδεει ")):
                cid=current_case_id(db)
            ...
            except Exception as exc: UI.err(str(exc)); UI.info("Πληκτρολογήστε help ή βοήθεια για παραδείγματα.")
    finally: db.close()
...
def build_parser() -> argparse.ArgumentParser:
    p=argparse.ArgumentParser(prog="Digital Footprint Finder Greek.py",formatter_class=argparse.RawDescriptionHelpFormatter,
        description="Digital Footprint Finder — καθοδηγούμενο, οργανωμένο ανά υπόθεση εργαλείο OSINT δημόσιων πηγών.")
    ...
    search=sub.add_parser("search",help="Αναζήτηση σε αποθηκευμένα δεδομένα υπόθεσης"); search.add_argument("query",nargs="+")
    web=sub.add_parser("websearch",help="Δημιουργία ενδείξεων από δημόσιες μηχανές αναζήτησης"); web.add_argument("query",nargs="+")
    ...
    api=sub.add_parser("api",help="Διαχείριση προαιρετικών API credentials χωρίς έκθεση secrets στο shell history")
...
def main(argv: Optional[list[str]]=None) -> int:
    ...
    if args.command=="settings":
        st=load_settings()
        if not args.key: print_result(st); return 0
        ...
        except Exception as exc: print(f"Σφάλμα: {exc}",file=sys.stderr); return 1
    if args.command=="api":
        if args.action=="list":
            print_result(api_status_report()); return 0
        provider=(args.provider or "").lower().strip()
        if provider not in API_PROVIDERS:
            print("Ο πάροχος πρέπει να είναι ένας από: " + ", ".join(API_PROVIDERS), file=sys.stderr); return 1
        if args.action=="remove":
            keys=load_api_keys(); keys.pop(provider,None); save_api_keys(keys); print("Το αποθηκευμένο API credential αφαιρέθηκε."); return 0
        value=getpass.getpass(f"{API_PROVIDERS[provider]['label']} API key/token (η εισαγωγή είναι κρυφή): ").strip()
        if not value:
            print("Δεν εισήχθη credential.",file=sys.stderr); return 1
        keys=load_api_keys(); keys[provider]=value; save_api_keys(keys); print("Το API credential αποθηκεύτηκε με ασφάλεια."); return 0
    ...
    try:
        ...
        if cid is None:
            print("Δεν υπάρχει αποθηκευμένη υπόθεση. Κάντε πρώτα μια έρευνα ή δώστε --case.",file=sys.stderr); return 1
